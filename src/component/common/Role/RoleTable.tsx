...
const RoleTable: React.FC<RoleTableProps> = ({
  ...
  const handleToggleDropdown = (roleId: number, type: "module" | "action") => {
    ...
    if (buttonElement && (!selectedRole || selectedRole.id !== roleId || selectedRole.type !== type)) {
      const position = calculateDropdownPosition(buttonElement);
      setDropdownPosition(position);
    }
  ...
  // Get assigned options for a role
  const getAssignedOptions = (roleName: string, type: "module" | "action") => {
    const mappings = groupedRoleMappings[roleName] || [];
    if (type === "module") {
      return [...new Set(mappings.map((m) => m.module_name))];
    } else {
      return [...new Set(mappings.map((m) => m.action_name))];
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      >
        <div className="max-h-[70vh] overflow-y-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentRoles.map((role, index) => {
                const assignedModules = getAssignedOptions(role.role_name, "module");
                const assignedActions = getAssignedOptions(role.role_name, "action");
                const selectedModules = roleModules[role.role_id] || [];
                const selectedActions = roleActions[role.role_id] || [];
                const hasValidSelection = validateSingleRoleSelection(role.role_id);

                // Combine assigned and selected for display
                const displayModules = [...new Set([...assignedModules, ...selectedModules])];
                const displayActions = [...new Set([...assignedActions, ...selectedActions])];
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <button
                        ...
                        onClick={() => handleToggleDropdown(role.role_id, "module")}
                        className={`inline-flex justify-between items-center w-full min-w-[180px] px-3 py-2 text-sm font-medium bg-white border border-gray-300 rounded-lg shadow-sm transition-all duration-150 ${
                          isReadOnly
                            ? "opacity-70 cursor-not-allowed text-gray-500"
                            : "text-gray-700 hover:bg-gray-50 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        }`}
                        ...
                        <div className="flex items-center space-x-2 flex-1 min-w-0">
                          {displayModules.length > 0 ? (
                            <>
                              ...
                              <span className="text-sm truncate">Modules</span>
                            </>
                          ) : (
                    ...
                    <td className="px-6 py-4">
                      <button
                        ...
                        onClick={() => handleToggleDropdown(role.role_id, "action")}
                        className={`inline-flex justify-between items-center w-full min-w-[180px] px-3 py-2 text-sm font-medium bg-white border border-gray-300 rounded-lg shadow-sm transition-all duration-150 ${
                          isReadOnly || displayModules.length === 0
                            ? "opacity-70 cursor-not-allowed text-gray-500"
                            : "text-gray-700 hover:bg-gray-50 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        }`}
                        disabled={isReadOnly || displayModules.length === 0}
                        title={
                          displayModules.length === 0
                            ? "Please select modules first"
                            : ""
                        ...
                        <div className="flex items-center space-x-2 flex-1 min-w-0">
                          {displayActions.length > 0 ? (
                            <>
                              ...
                              <span className="text-sm truncate">Actions</span>
                            </>
                          ) : (
                            <span className="text-sm text-gray-500">
                              {displayModules.length === 0
      ...
      {/* Module Dropdown */}
      {selectedRole && selectedRole.type === "module" && (
        <div 
        ...
        >
          ...
          >
            <MultiSelectDropdown
              ...
              options={moduleOptions}
              selectedOptions={roleModules[selectedRole.id] || []}
              preSelectedOptions={getAssignedOptions(
                currentRoles.find(r => r.role_id === selectedRole.id)?.role_name || "",
                "module"
              )}
              onApply={(selected) => handleApply(selectedRole.id, selected, "module")}
              onReset={() => handleApply(selectedRole.id, [], "module")}
      ...
      {/* Action Dropdown */}
      {selectedRole && selectedRole.type === "action" && (
        <div 
          className="fixed inset-0 z-50"
          style={{ pointerEvents: 'none' }}
        >
          ...
          >
            <MultiSelectDropdown
              ...
              selectedOptions={roleActions[selectedRole.id] || []}
              preSelectedOptions={getAssignedOptions(
                currentRoles.find(r => r.role_id === selectedRole.id)?.role_name || "",
                "action"
              )}
              onApply={(selected) => handleApply(selectedRole.id, selected, "action")}
              onReset={() => handleApply(selectedRole.id, [], "action")}
              onClose={closeDropdown}
              maxHeight={400}
            />
          </div>
