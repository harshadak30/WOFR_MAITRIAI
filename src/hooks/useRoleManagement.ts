...
// //       setTotalPages(
// //         data.meta.total_pages || Math.ceil((data.meta.total_items || rolesData.length) / limit)
// //       );
// //     } catch (error) {
...
// //       .filter(({ module_name }) => !seen.has(module_name) && seen.add(module_name))
...
// //     }
// //     // Don't automatically close dropdown - let the component handle it
// //   };
...
// //     return totalModules.length > 0 && totalActions.length > 0 && 
...
// //       setMessage(`Please select both modules and actions for: ${role.role_name}`);
// //       return;
// //     }
...
// //       setMessage(
// //         `Failed to create role: ${error?.response?.data?.detail || "Unknown error"}`
// //       );
// //     } finally {
...
//     const assignedPairs = assignments.map(a => `${a.module_name}-${a.action_name}`);
    
//     // Get all possible module-action pairs
//     const allPairs = moduleActionPairs.map(pair => `${pair.module_name}-${pair.action_name}`);
...
//       !fullyAssignedModules.has(module.id) || selectedModules.includes(module.id)
//     );
//   };
...
//       !assignedActions.includes(action.id) || currentlySelectedActions.includes(action.id)
//     );
//   };
...
const handleToggleRoleStatus = async (roleId: number) => {
  ...
  try {
    await axios.patch(`/api/v1/roles-status/${roleId}`, {}, {
      headers: getAuthHeaders()
    });
