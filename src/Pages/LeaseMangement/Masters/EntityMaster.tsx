...
const EntityMaster: React.FC = () => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false);
  // const [isSubmitting, setIsSubmitting] = useState(false);
  //  const {entities,fetchEntityMaster}=useMaster();
  // Static data for dropdowns
  const {
    entities,
    fetchEntityMaster,
  ...
  // };
  const onSubmit = async (data: FormData) => {
    const success = await EntitySubmit(data);
    if (success) {
      fetchEntityMaster(); // Refresh the entity list
      // closeModal();
      setIsModalOpen(false);
    }
  };
                  ...
                  <div>
                    ...
                    {department.length === 0 && (
                      <p className="text-yellow-600 text-sm mt-2">
                        Note: If you want to select a department, please create
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {errors.Ownership_share && (
                      <p className="text-red-500 text-sm mt-1">
                        {errors.Ownership_share.message}
                      </p>
                    )}
                  </div>
                  {/* Parent Organization */}
