...
//  const {LessorData,fetchLessors}=useMaster();
  // Static data for dropdowns
  // const relationshipTypes = [
  //   { id: 1, name: "Subsidiary" },
  //   { id: 2, name: "Branch" },
...
<form onSubmit={handleSubmit(onSubmit)} className="p-6">
  ...
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
            errors.Lessor_Name ? "border-red-300 bg-red-50" : "border-gray-300"
          }`}
          placeholder="Enter Lessor name"
        />
        {errors.Lessor_Name && (
          ...
          {...register("Vendor_Code", {
            ...
            validate: {
              noConsecutiveSpecialChars: (value) => 
                !/([-_]{2,})/.test(value) || "No consecutive special characters allowed",
              startsWithLetter: (value) =>
                /^[A-Za-z]/.test(value) || "Must start with a letter"
            }
          })}
          className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
            errors.Vendor_Code ? "border-red-300 bg-red-50" : "border-gray-300"
          }`}
          placeholder="Enter Vendor Code (e.g., VEN001)"
        />
        {errors.Vendor_Code && (
      ...
      <div>
        ...
        <Controller
          ...
          rules={{ 
            ...
            validate: (value) => 
              vatOptions.some(option => option.value === value) || "Invalid VAT option selected"
          }}
          render={({ field }) => (
            <select
              {...field}
              className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                errors.VAT_application ? "border-red-300 bg-red-50" : "border-gray-300"
              }`}
            >
              <option value="">Select VAT Application</option>
              {vatOptions.map((option) => (
          ...
          {...register("Email", {
            ...
            validate: {
              noDisposableEmail: (value) =>
                !/^(guerrillamail|mailinator|tempmail|10minutemail|yopmail)\./i.test(value.split('@')[1]) ||
                "Disposable email addresses are not allowed",
              hasValidDomain: (value) => 
                /\.(com|org|net|edu|gov|co|io|ai|biz|info)$/i.test(value.split('@')[1]) ||
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Tax Deduction Application <span className="text-red-500">*</span>
        </label>
        <Controller
          control={control}
          name="Tax_deduction_Application"
          rules={{
            required: "Tax deduction application is required",
            validate: (value) => 
              taxDeductionOptions.some(option => option.value === value) || "Invalid tax deduction option"
          }}
          render={({ field }) => (
            <select
              {...field}
              className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                errors.Tax_deduction_Application ? "border-red-300 bg-red-50" : "border-gray-300"
              }`}
            >
              <option value="">Select Tax Deduction</option>
              {taxDeductionOptions.map((option) => (
    ...
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Vendor Bank Name <span className="text-red-500">*</span>
        </label>
        <input
          {...register("Vendor_bank_name", {
            required: "Vendor bank name is required",
            ...
            pattern: {
              value: /^[A-Za-z\u00C0-\u017F\s\-,.&]+$/,
              message: "Bank name can only contain letters, spaces, and basic punctuation",
            },
            validate: (value) => {
              const trimmedValue = value.trim();
              if (trimmedValue.length < 2) {
                return "Bank name must be at least 2 non-whitespace characters";
              }
              return true;
            }
          })}
          className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
            errors.Vendor_bank_name ? "border-red-300 bg-red-50" : "border-gray-300"
          }`}
          placeholder="Enter Vendor bank name"
        />
        {errors.Vendor_bank_name && (
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Vendor Registration Number <span className="text-red-500">*</span>
        </label>
        <input
          {...register("Vendor_registration_number", {
            required: "Vendor registration number is required",
            ...
            validate: {
              noConsecutiveSpecialChars: (value) => 
                !/([-_]{2,})/.test(value) || "No consecutive special characters allowed",
              hasRequiredDigits: (value) => 
                 (value.match(/\d/g)?.length >= 3) || "Must contain at least 3 digits"
            }
          })}
          className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
            errors.Vendor_registration_number ? "border-red-300 bg-red-50" : "border-gray-300"
          }`}
          placeholder="Enter Vendor registration number"
        />
        {errors.Vendor_registration_number && (
          ...
          {...register("Tax_Identification_number", {
            ...
            validate: {
              noConsecutiveSpecialChars: (value) => 
                !/([-_]{2,})/.test(value) || "No consecutive special characters allowed",
              hasRequiredDigits: (value) => 
                (value.match(/\d/g)?.length >= 8 )|| "Must contain at least 8 digits"
            }
          })}
          className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
            errors.Tax_Identification_number ? "border-red-300 bg-red-50" : "border-gray-300"
          }`}
          placeholder="Enter Tax Identification number"
        />
        {errors.Tax_Identification_number && (
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Vendor Bank Account Number <span className="text-red-500">*</span>
        </label>
        <input
          {...register("Vendor_Bank_Account_Number", {
            required: "Vendor bank account number is required",
            ...
            validate: {
              noConsecutiveHyphens: (value) => 
                !/(\-{2,})/.test(value) || "No consecutive hyphens allowed",
              hasRequiredDigits: (value) => 
                (value.replace(/\D/g, '').length >= 8) || "Must contain at least 8 digits",
              validIBANFormat: (value) => 
                !/^[A-Za-z]{2}/.test(value) || "For IBAN, use country code at start (e.g., GB29NWBK60161331926819)"
            }
          })}
          className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
            errors.Vendor_Bank_Account_Number ? "border-red-300 bg-red-50" : "border-gray-300"
          }`}
          placeholder="Enter Bank Account Number (e.g., 12345678 or GB29NWBK60161331926819)"
        />
        {errors.Vendor_Bank_Account_Number && (
          <p className="text-red-500 text-sm mt-1 flex items-center gap-1">
            <svg
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Related Party Relationship <span className="text-red-500">*</span>
        </label>
        <Controller
          control={control}
          name="relatedPartyRelationship"
          rules={{
            required: "Related party relationship is required",
            validate: (value) => 
              relationshipTypes.some(type => type.id.toString() === value) || "Invalid relationship type"
          }}
          render={({ field }) => (
            <select
              {...field}
              className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                errors.relatedPartyRelationship ? "border-red-300 bg-red-50" : "border-gray-300"
              }`}
            >
              <option value="">Select relationship type</option>
              {relationshipTypes.map((type) => (
