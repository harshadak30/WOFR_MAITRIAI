  ...
  const fetchModules = async (page: number, limit: number, search?: string) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        console.log("Modules after setting:", Array.from(moduleMap.values())); // Check if user_id exists
...
const handleToggleChange = async (moduleId: number, user_id: string) => {
  console.log(moduleId);
  console.log(user_id);
  setToggleLoading(moduleId);
  try {
    const response = await axios.patch(
      `api/v1/user-role-assignment-toggle?user_id=${user_id}&module_id=${moduleId}`
    );
    ...
    if (response.data.success) {
      ...
      toast.success(
        response.data.message || `Module status updated successfully`
      );
    } else {
      toast.error(response.data.message || "Failed to toggle module status");
    }
  } catch (error) {
