...
const grouped = allAssignments
  .filter((assignment: AssignedRole) => assignment.tenant_user_id === userId)
  .reduce((acc: any, assignment: AssignedRole) => {
    ...
    if (!acc[key]) {
      acc[key] = {
        ...
        module_name: assignment.screen_data.module_action_pair.module.module_name,
        actions: {},
    ...
    const actionName = assignment.screen_data.module_action_pair.action.action_name;
...
setGroupedAssignments(groupedArray);
  ...
  <tbody className="bg-white divide-y divide-gray-200">
    {groupedAssignments.map((group, index) => (
      <tr key={`${group.role_name}-${group.module_name}-${index}`} className="hover:bg-gray-50">
          ...
          <select className="border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            {group.actions.flatMap(action => 
              action.sub_actions.map((subAction, subIndex) => (
                <option key={`${action.action_name}-${subIndex}`} value={subAction.sub_action_name}>
